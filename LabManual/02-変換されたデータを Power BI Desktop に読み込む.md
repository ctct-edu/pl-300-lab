...
1. この演習を完了するには、まず Web ブラウザーを開き、次の URL を入力して zip フォルダーをダウンロード、展開します。
    ...
    以前のラボ実施状態が保持されている場合はそこから引き続き実施できます。以前のラボの実施状態が失われている、完了していない場合は **02-Starter-Sales Analysis.pbix** ファイルを開いてラボを開始します。
...
1. Power Queryエディターでクエリを編集します。エディターが閉じている場合、 **[Power Query エディター]** ウィンドウを開くには、**[ホーム]** リボン タブの **[クエリ]** グループ内から、**[データの変換]** アイコンを選択します。
...
1. 特定の列を見つけるには、 **[ホーム]** リボン タブの **[列の選択]** の下矢印を選択して、 **[列に移動]** を選択します。
    ...
    [列に移動] は、列が多いときに特定の列を参照するのに便利な機能です。水平方向にスクロールして列を見つけるよりも早く簡単に見つけることができます。
...
1. **SalesPersonFlag** 列を見つけたら、営業担当者(この列でフラグが**TRUE**の従業員) のみが選択されるようにこの列をフィルター処理（列名右側の下向き三角形）して、 **[OK]** をクリックします。
...
2. **FinishedGoodsFlag** 列を見つけ、列をフィルター処理して、完成品 (つまり、TRUE) である製品を取得します。

3. 以下を**除く**すべての列を削除します。
...
4. **DimProductSubcategory** 列は関連テーブルを表していることに注意してください (**Value** リンクが含まれています)。

5. **DimProductSubcategory** 列ヘッダーで、列名の右側にある展開ボタンを選択します。

    ![列展開アイコン](./image/02/02-load-data-with-power-query-in-power-bi-desktop_image31.png)

6. 列の一覧を表示し、 **[すべての列の選択]** ボックスを解除します。

7. **EnglishProductSubcategoryName** と **DimProductCategory** を選択し、 **[元の列名をプレフィックスとして使用します]** チェック ボックスをオフにしてから、 **[OK]** を選択します。
...
8. 変換の結果、2 つの列が追加され、**DimProductSubcategory** 列が削除されたことがわかります。

9. **DimProductCategory** 列を展開し、**EnglishProductCategoryName** 列のみを追加します。

10. 次の 4 つの列の名前を変更します。
...
11. ステータス バーで、クエリに 6 列 397 行が含まれていることを確認します。
...
1. 新たに **属性** 列、**値** 列として表示が変更されていることを確認してください。

1. **値** 列にフィルターを適用して、ハイフン (-) 値を削除します。
...
1. **[列1]** 列の最初のグリッド セルに、「**2017/7**」と入力してから、**Enter** キーを押します。
...
1. **[クエリのプロパティ]** ウィンドウで、**[読み込みを有効にする]** チェックボックスをオフにします。
