...
1. この演習を完了するには、まず Web ブラウザーを開き、次の URL を入力して zip フォルダーをダウンロード、展開します。
    ...
    以前のラボ実施状態が保持されている場合はそこから引き続き実施できます。以前のラボの実施状態が失われている、完了していない場合は **03-Starter-Sales Analysis.pbix** ファイルを開いてラボを開始します。
...
1. すべてのテーブル フィールドを表示するには、 **[データ]** ペインで何も表示されていない領域を右クリックし、 **[すべて展開する]** を選択します。
...
1. **[新しいリレーションシップ]** をクリックして新たにリレーションシップを作成します。
...
1. **[保存]** をクリックし、 **[リレーションシップの管理]** ウィンドウで、新しいリレーションシップが表示されていることを確認して、 **[閉じる(close)]** を選択します。
...
1. 作成された階層が選択されている状態で、 **[プロパティ]** ペイン ( **[データ]** ペインの左側) で、 **[名前]** ボックスのテキストを「**Products**」に置き換えます。
...
1. 自動/日付時刻をオフにするには、 **[ファイル] > [オプションと設定] > [オプション] > [現在のファイル]** グループに移動し、 **[データの読み込み]** を選択します。
    1. **[タイム インテリジェンス]** セクションで、**[自動の日付/時刻]** チェック ボックスをオフにします。
