...
1. この演習を完了するには、まず Web ブラウザーを開き、次の URL を入力して zip フォルダーをダウンロードします。
   ...
   以前のラボ実施状態が保持されている場合はそこから引き続き実施できます。以前のラボの実施状態が失われている、完了していない場合は **08-Starter-Sales Analysis.pbix** ファイルを開いてラボを開始します。
...
![ロゴ、2 つのスライサー、3 つのビジュアルで構成されているページ 1 のイメージ。](./image/06/PowerBI_Lab08A_image9.png)

1. Power BI Desktop で、ページの名前を変更するには、左下にある **[Page 1]** を右クリックしてから、そのページを **Overview** という**名前に変更**します。

    *ヒント:ページ名をダブルクリックして名前を変更することも可能です。"*

1. 画像を追加するには、**[挿入]** リボン タブで、**[要素]** グループ内の **[イメージ]** を選択します。

    ![画像 1](./image/06/07-design-report-in-power-bi-desktop_image15.png)

1. **[形式のイメージ]** ペインで [スタイル]→[イメージ]をそれぞれ展開し、**[イメージ]** の項目から **C:\Users\ctct\Downloads\08-design-report** フォルダー内の**AdventureWorksLogo.jpg** ファイルを選択します。
...
1. **[データ]** ペインから、**Sales \| Profit Margin** フィールドを **[線の Y 軸]** ウェル/領域にドラッグ&ドロップします。
...
1. **[バー]** 、 **[カラー]** グループの順に展開し、 **[カラー]** プロパティを適切な色に設定します (縦棒グラフや折れ線グラフを補完する色)。
...
1. **[書式]** ペインを使用して、[すべて選択] オプションを有効にします ( **[選択項目]** グループ)。
...
1. **[フィルター]** ペイン (**[視覚化]** ペインの左側) で、**[このページでのフィルター]** 領域を確認します。

     ![画像 57](./image/06/07-design-report-in-power-bi-desktop_image44.png)
...
1. **[カテゴリ]** カード直下で、**[このページでのフィルター]** ウェル/領域に次の各 **Product** テーブル フィールドを追加して、それぞれを折りたたみます。
     ...
     ![画像 60](./image/06/07-design-report-in-power-bi-desktop_image46.png)
...
このタスクでは、3つ目のレポート ページをデザインします。 設計が完了すると、ページは次のようになります。
 ...
 ![3 つのビジュアルで構成されたページ 3 のイメージ。](./image/06/07-design-report-in-power-bi-desktop_image47.png)
...
1. **[Michael Blythe]** を選択します。 **[My Performance]** レポート ページは、Michael Blythe のみのデータを表示するためにフィルター処理されています。
...
1. **[複数の行カード]** 視覚化を追加し、スライサーの右側でページの残りの幅を埋めるようにサイズと位置を変更します。
     ...
     > ***[ビジュアル化]** ウィンドウに **[複数行カード]** ビジュアルが表示されない場合は、ビジュアルの下にある省略記号 (**...**) を選択し、**[既定の視覚化の復元]** 実行します。**[複数行カード]** が選択できるようになります。*
...
1. 視覚化の書式を設定します。
     ...
     - ビジュアルの書式設定から、**[吹き出し]** グループで **[フォント]** プロパティを **28 pt** に増やします

     - **[全般] > [効果] > [背景]** グループで、 **[カラー]** を薄い灰色 ("白、20% 暗い" など) に設定して、コントラストを指定します
...
1. 次のフィールドをビジュアル ウェル/領域に追加します。
     ...
     - X 軸: **Sales \| Sales** および **Targets \| Target**
         ...
         ![画像 80](./image/06/07-design-report-in-power-bi-desktop_image56.png)
...
1. **[ホーム]** リボン タブで、**[共有]** グループの **[発行]** を選択します。変更の保存やクエリの適用に関するダイアログボックスが表示された場合は、保存や適用を選択します。
   
    発行を行うにはMicrosoftアカウントを使用してログインする必要があるため、以下の情報を使用してサインインします。

    | 項目           | 値                                                           |
   | -------------- | ------------------------------------------------------------ |
   | メールアドレス | pbiuser[XX]@ctctedu.onmicrosoft.com<br />※[XX]の部分は演習用仮想マシンの接続先情報と同じ数字2桁を使用してください |
   | パスワード     | 演習用仮想マシンと同値                                       |
...
1. Microsoft Edge ブラウザー ウィンドウの Power BI サービスで画面左側の **[ナビゲーション]** ウィンドウから、**[マイワークスペース(My workspace)]** を展開します。

1. ワークスペースのコンテンツを確認します。
    1. ''ワークスペースに存在できる項目は 4 種類あります。ここでは、**レポート**と**セマンティックモデル**について説明します。''
    1. ''セマンティックモデルが表示されない場合は、Microsoft Edge ブラウザーを最新の情報に更新する必要がある場合があります。''
    1. *Power BI Desktop ファイルを発行したときに、データ モデルはセマンティックモデルとして発行されました。*
...
1. もう一度ビジュアルの 1 つをポイントし、右上にある省略記号 (...) を選択して、メニュー オプションに注目します。個別のビジュアルに対して実行できるアクションが表示されています。

1. 左側の **[ページ]** ペインで、**Profit** ページを選択します。

      ![画像 84](./image/06/07-design-report-in-power-bi-desktop_image68.png)

1. **Region** スライサーは、**[概要]** ページの **[Region]** スライサーとは異なる項目が選択をされていることに注目してください。
...
1. メニュー バーの右上にある**四角いアイコン** もしくは **[View]** （表示）をクリックして、**[Fullscreen]** （全画面表示）を選択します。

1. スライサーを変更し、ページをクロスフィルター処理して、ページを操作します。

1. ウィンドウの最下部には、ページの変更、ページ間での前後への移動、または全画面表示モードを終了するためのコマンドがあります。

1. Escキーを入力するか、画面上部にマウスカーソルを運ぶと表示される×アイコンをクリックして、全画面表示モードを終了します。
...
ワークスぺースに戻るには、ウィンドウの 左側下部に表示されている **[マイワークスペース(My workspace)]** を選択します。
