...
1. この演習を完了するには、まず Web ブラウザーを開き、次の URL を入力して zip フォルダーをダウンロードします。
     ...
     以前のラボ実施状態が保持されている場合はそこから引き続き実施できます。以前のラボの実施状態が失われている、完了していない場合は **09-Starter-Sales Analysis.pbix** ファイルを開いてラボを開始します。
...
1. **[スライサーの同期]** ページを閉じるには、再度 **[表示]** リボン タブの **[ペインを表示する]** グループ内の **[スライサーの同期]** をクリックします。
...
1. **[Product Details]** ページ タブを右クリックし、**[非表示]** を選択します。
...
1. **[全般]>[効果] > [背景]** プロパティを、''白、20% 暗い'' などの灰色の薄い網掛けに設定します。
...
1. ビジュアルの書式オプションを構成し、 **[値]** セクションで、 **[フォント]** プロパティを **[20pt]** に設定します。
...
1. 次のように、最初のルール (赤色のひし形) を構成します。
    ...
    - 2 つ目のコントロールで値(0が入力されている箇所)を削除します
    - 3 つ目のコントロールで**[パーセント]** を **[数値]** に変更します
...
1. **[背景色 - Color]** ウィンドウの **[スタイルの書式設定]** ドロップダウン リストで、 **[フィールド値]** を選択します。
...
1. ボタンを選択し、 **[書式設定ボタン]** ペインで、 **[ボタン]** を選び、 **[スタイル]** セクションを展開し、 **[テキスト]** プロパティを **[オン]** にします。

1. **[テキスト]** セクションを展開し、 **[テキスト]** ボックスに「**Bar Chart**」と入力します。

1. **[塗りつぶし]（フィル）** セクションを**[オン]** にして、**[カラー]** を使用して塗りつぶしの色を白色以外に設定します。
...
1. コピーと貼り付けを使用してボタンのコピーを作成し、次のように新しいボタンを構成します。
    ...
    *ヒント:コピーと貼り付けのショートカット コマンドは、**Ctrl+C** キーに続いて **Ctrl+V** キーです。"*
    ...
    - **[ボタン テキスト]** プロパティを **[Column Chart ON (縦棒グラフ オン)]** に設定します
...
1. **[Power BI に発行]** ウィンドウで、**[マイワークスペース(My Workspace)]** が選択されていることを確認します。
