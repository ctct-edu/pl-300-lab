...
1. **[インポート] > [レポート、ページ割り付けレポート、またはブック] > [コンピューターから]** の順に選択します。
...
1. **[Region]** スライサーを **[すべて選択(Select all)]** に設定します。
...
1. ダッシュボードを作成してビジュアルをピン留めするには、 **[ Month による Sales および Profit Margin]** (列/行) 視覚化にカーソルを合わせ、プッシュピンを選択します。

    ![画像 43](./image/09/09-create-power-bi-dashboard_image18.png)

1. **[Pin to dashboard(ダッシュボードにピン留めする)]** ウィンドウで、 **[ダッシュボード名(Dashboard name)]** ボックスに「**Sales Monitoring**」と入力してから、 **[ピン留め]** を選択します。
...
1. 質問に **in year FY2020** (FY2020 年度の) を追加して、回答内容を修正します。

1. 応答が **$33M（もしくは$33,139,748）** になったことに注意してください。
...
1. ダッシュボードにタイルをピン留めするように求めるメッセージが表示されたら、リストから先ほど作成したダッシュボードを選択して **[Pin(ピン留め)]** を選択します。
...
1. (右側にある) **[タイルの追加]** ペインで、 **[image(イメージ)]** タイルを選んでから、 **[Next(次へ)]** 選択します。

1. **[画像タイルの追加]** ペインの **[URL]** ボックスに、**C:\Users\ctct\Downloads\12-create-dashboard\AdventureWorksLogo_DataURL.txt** ファイルを開くと確認できる長大なURL （ファイルパスではなくテキストファイル内のURLをすべてコピー）をすべて入力してから、 **[適用]** を選択します。
...
1. **[Sales, Profit Margin]** タイルの詳細を編集します。
...
1. エクスプローラーで、展開したZipフォルダ内にある **UpdateDatabase-2.sql** を右クリック→プログラムから開く→メモ帳で開き、以下の内容を変更します。
    ...
    変更後：**C:\Users\ctct\Downloads\12-create-dashboard\ResellerSales_202006.csv**

1. 同じくフォルダ内にある **UpdateDatabase-2-AddSales.ps1** ファイルを右クリックし、 **[PowerShell で実行]** を選択します。
...
このタスクでは、このラボの冒頭でPower BIサービスにインポートした **12-Starter-Sales Analysis.pbix** ファイルをPower BI Desktopで開き、データ更新を実行してそのセマンティックモデルを **Sales Analysis** ワークスペースにアップロードします。

1. Power BI Desktopの **[データ]** ペインで、**Sales** テーブルを右クリックして **[データの更新]** を選択します。

    クエリの適用を求められた場合は、"変更を破棄"してから再度実施してください。

    データ接続に関する内容についてもそのまま接続して続行します。
